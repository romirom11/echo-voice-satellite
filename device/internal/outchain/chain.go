...
func (p Params) Equal(o Params) bool {
	...
	return p.GainDB == o.GainDB &&
		...
		p.GuardDB == o.GuardDB &&
		p.BassShelfHz == o.BassShelfHz &&
		p.SubsonicHz == o.SubsonicHz
}
...
func (s *stages) clone(p Params) *stages {
	...
	c.eq.SetSettings(p.Bands, p.Loudness, true, p.BassShelfHz, p.SubsonicHz)
...
func (e *EQ) clone() *EQ {
	c := &EQ{fs: e.fs, flat: e.flat, subsonic: e.subsonic,
		bassShelfHz: e.bassShelfHz, subsonicHz: e.subsonicHz}
